...
export const getBookingById = async (id) => {
  try {
    return await Booking.findById(id).populate('user', 'fullName email phoneNumber');
  } catch (error) {
...
export const getUpcomingBookings = async () => {
  try {
    ...
    }).populate('user', 'fullName email phoneNumber').sort({ date: 1, time: 1 });
  } catch (error) {
...
export const getAllBookings = async () => {
  try {
    return await Booking.find({}).populate('user', 'fullName email phoneNumber').sort({ createdAt: -1 });
  } catch (error) {
